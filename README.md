...
Для получения последних изменений и обновления приложения:
1. Выполните `git pull`
2. Пересоберите сервисы: `docker compose up --build -d`

## Важно про данные и резервное копирование

Команда `docker compose down -v` удаляет контейнеры **и volume с базой**, это приведёт к потере локальных данных.

Для безопасного управления данными встроен механизм Экспорта/Импорта:
- **Экспорт**: Скачивает полное состояние (counts, историю) в единый JSON-файл. Рекомендуется делать регулярно.
- **Импорт**: Выполняет объединение (merge) данных из JSON поверх текущей базы по имени типа. При совпадении имени счётчики суммируются, история дополняется.

Перед потенциально рискованными операциями всегда делайте **Экспорт** данных из приложения.
