...
- Финальная приборка репозитория (убедились, что линтеры и форматтеры зелёные).
- Зафиксированы security hardening доработки.
- В `BACKLOG.md` задача `CLEAN-006` помечена выполненной.
- Добавлены артефакты процесса: `tasks/CLEAN-006.md`.

### DOCS-002

- Добавлен файл `AGENTS.md` в корень репозитория как высший источник правды для AI-агентов.
- Зафиксированы инварианты проекта, порядок стадий, DoD.
- В `BACKLOG.md` задача `DOCS-002` помечена выполненной.
- Добавлены артефакты процесса: `tasks/DOCS-002.md`, `ADR/ADR-0036-agents-md-docs-002.md`.

### DOCS-001

- Обновлён `README.md` с подробным описанием процесса обновления приложения (через git pull & docker compose).
- Добавлена явно выделенная секция про экспорт и импорт, с объяснением поведения merge по имени.
- В `BACKLOG.md` задача `DOCS-001` помечена выполненной.
- Добавлены артефакты процесса: `tasks/DOCS-001.md`.

### OPS-001

- Контейнер Nginx переведён на использование `nginxinc/nginx-unprivileged:1.27-alpine` (least privilege/работает не от root).
- Для Nginx изменён прослушиваемый порт на `8080` (внутри контейнера) и добавлен healthcheck `wget`.
- Backend (FastAPI) контейнер переведён на выполнение от созданного непривилегированного пользователя `appuser`.
- В `BACKLOG.md` задача `OPS-001` помечена выполненной.
- Добавлены артефакты процесса: `tasks/OPS-001.md`, `ADR/ADR-0035-container-hardening-ops-001.md`.

### SEC-002

- В backend добавлен лимит на размер (10MB) JSON payload при `POST /api/v1/import` через `Depends(_check_payload_size)` функцию.
- Бизнес-ошибка `ERR_PAYLOAD_TOO_LARGE` добавлена в схемы и локализована во фронтенде (включая fallback на английский).
- В `BACKLOG.md` задача `SEC-002` помечена выполненной.
- Добавлены артефакты процесса: `tasks/SEC-002.md`, `ADR/ADR-0034-import-payload-size-limit-sec-002.md`.

### SEC-001

- Проведён аудит безопасности: проверено отсутствие секретных ключей или паролей в кодовой базе (всё через `.env` и пример `env.example`).
- Проверено, что доступные по умолчанию логи (ошибки валидации) не сливают наружу сырые данные импорта/экспорта.
- `.env` корректно изолирован с помощью `.gitignore`.
- В `BACKLOG.md` задача `SEC-001` помечена выполненной.
- Добавлены артефакты процесса: `tasks/SEC-001.md`, `ADR/ADR-0033-secrets-and-logs-sec-001.md`.### CLEAN-005

- Выполнен рефакторинг тестовых фикстур: добавлена зависимость `db_engine` для корректной работы с PostgreSQL вместо in-memory SQLite и прямого создания engine внутри тестов.
- Обновлены все backend тесты:
  - вместо `database_url` тесты теперь принимают фикстуры `client` и `db_engine`;
  - удалено ручное создание `engine` и `TestClient` из самих тестов;
  - удалены блоки `try...finally` с ручной очисткой кеша настроек, так как это теперь делается внутри фикстуры `client`.
- Исправлены баги в тестах, возникшие из-за миграции на реальную БД (Postgres вместо SQLite) и конфликтов имен переменных.
- Linter (`ruff`, `flake8`) проверен — 0 ошибок.
- Полный backend quality gate пройден: 49 passed. Все тесты стабильно проходят.
- В `BACKLOG.md` задача `CLEAN-005` помечена выполненной.
...
- Расширены и доработаны тесты импорта в `backend/tests/test_import_api.py`:
  - вынесен helper `_all_stages_zero` и константа `ERR_INVALID_IMPORT_FORMAT` для устранения дублирования;
  - добавлен тест на создание нового типа через импорт (pure creation, без merge);
  - добавлен тест на повторный импорт — counts удваиваются, история дублируется (no dedupe per PRD);
  - добавлен тест на импорт пустого списка типов — noop, без побочных эффектов;
  - добавлен тест на полностью невалидный payload (отсутствует ключ `types`);
  - добавлен тест на невалидное имя стадии в `stage_counts`;
  - добавлен тест на невалидное имя стадии в `history`;
  - добавлен тест на отрицательный count в `stage_counts`;
  - добавлен тест на лишние поля в payload (`extra="forbid"`);
  - добавлен тест на атомарность: изменения валидного типа откатываются, если последующий тип невалиден.
- Исправлены pre-existing lint-ошибки в тестах:
  - удалены неиспользуемые импорты `pathlib.Path` из 5 файлов;
  - добавлен `noqa: E402` для намеренного позднего импорта в `conftest.py`.
- Полный backend quality gate пройден: 49 passed (40 существующих + 9 новых), lint 0 ошибок.
- В `BACKLOG.md` задача `QA-004` помечена выполненной.
- Добавлены артефакты процесса:
  - `ADR/ADR-0031-import-tests-qa-004.md`;
  - `tasks/QA-004.md`.

### QA-003

- Расширены и доработаны тесты группировки истории в `backend/tests/test_type_history_api.py`:
  - существующие тесты рефакторены: вынесен helper `_create_type_and_seed_history` для устранения дублирования setup-логики;
  - добавлен тест на точную границу 300 секунд (два события) → группируются;
  - добавлен тест на точную границу 301 секунда (два события) → не группируются;
  - добавлен тест на скользящее окно: цепочка событий по 299с друг от друга, суммарный span > 300с от первого → все в одной группе (сравнение идёт с предыдущим событием, не с началом группы);
  - добавлен тест на пустую историю → пустой список групп;
  - добавлен тест на единственное событие → одна группа;
  - добавлен тест на несколько чередующихся переходов → корректное количество групп;
  - добавлен тест на одновременные события (0 секунд разницы) → группируются.
- Полный backend quality gate пройден: 40 passed (33 существующих + 7 новых).
- В `BACKLOG.md` задача `QA-003` помечена выполненной.
- Добавлены артефакты процесса:
  - `ADR/ADR-0030-history-grouping-tests-qa-003.md`;
  - `tasks/QA-003.md`.

### QA-002

- Добавлены тесты конкурентности и инвариантов для endpoint `POST /api/v1/types/{id}/move` в `backend/tests/test_move_concurrency.py`.
- Тесты конкурентности (3 шт.):
  - 20 параллельных single-unit запросов при 10 доступных — ровно 10 успехов, 10 отказов;
  - 6 параллельных batch-запросов по 5 шт. при 15 доступных — ровно 3 успеха;
  - конкурентные перемещения из разных стадий — независимая обработка.
- Тесты инвариантов (5 шт.):
  - граничный кейс drain-to-zero;
  - пропуск промежуточных стадий (IN_BOX → DONE);
  - отклонение перехода в ту же стадию;
  - 404 для несуществующего типа;
  - multi-step sequential moves.
- Подтверждена корректность пессимистической блокировки `SELECT ... FOR UPDATE`: counts не уходят в минус при параллельной нагрузке.
- Полный backend quality gate: 33 passed.
- В `BACKLOG.md` задача `QA-002` помечена выполненной.
- Добавлены артефакты процесса:
  - `ADR/ADR-0029-move-concurrency-tests-qa-002.md`;
  - `tasks/QA-002.md`.

### QA-001

- Добавлена инфраструктура для запуска backend-тестов в изолированном окружении с реальной БД Postgres (`docker-compose.test.yml`).
- Команда `make backend-test` перенастроена на запуск тестов в контейнере `test-app`.
- Выполнен рефакторинг инфраструктуры тестов: in-memory SQLite заменен на реальный инстанс Postgres через фикстуру `database_url`.
- Для всех интеграционных тестов прогон миграций `alembic upgrade head` теперь выполняется автоматически с созданием схемы с нуля.
- В `BACKLOG.md` задача `QA-001` помечена выполненной.
- Добавлены артефакты процесса:
  - `tasks/QA-001.md`.
...
- Доработан компонент `HistorySection` в `frontend/src/pages/TypeDetailsPage.tsx`:
  - добавлен обратный хронологический порядок отображения групп (newest first) — стандартный UX-паттерн для activity logs;
  - добавлено локаль-зависимое форматирование дат через `i18n.language` (`ru-RU` / `en-US`) вместо браузерной локали;
  - добавлен семантический атрибут `dateTime` к элементу `<time>` для доступности.
- Пограничные кейсы подтверждены:
  - пустая история: корректный empty state с i18n;
  - единственная группа: отображается без артефактов;
  - переключение языка: формат дат обновляется мгновенно.
- Полный frontend quality gate пройден: `tsc --noEmit` (0 ошибок), `eslint .` (0 ошибок), `vite build` (dist/ собран).
- Backend тесты: 25 passed.
- В `BACKLOG.md` задача `FE-006` помечена выполненной.
- Добавлены артефакты процесса:
  - `ADR/ADR-0026-history-ui-display-fe-006.md`;
  - `tasks/FE-006.md`.

### FE-005

- Исправлена карточка типа (TypeDetailsPage): отображение counts и форма перемещения.
- Исправлен баг синхронизации `fromStage`:
  - ранее при загрузке типа с IN_BOX = 0 select «Откуда» показывал невалидное значение, не совпадающее с доступными опциями;
  - добавлен `useEffect`, автоматически выбирающий первую стадию с ненулевым count.
- Удалён вводящий в заблуждение fallback-список:
  - ранее при отсутствии доступных стадий (все counts = 0) dropdown показывал все стадии кроме DONE, хотя кнопка была заблокирована;
  - теперь при отсутствии доступных перемещений отображается информативное сообщение.
- Добавлен empty state для формы перемещения с i18n-ключом `moveNoAvailable` (RU/EN).
- Подключено обновление секции истории после перемещения (передан `refreshKey` в `HistorySection`).
- Добавлен CSS-класс `.section-empty` для пустых состояний внутри секций.
- Полный frontend quality gate пройден: `tsc --noEmit` (0 ошибок), `eslint .` (0 ошибок), `vite build` (dist/ собран).
- Backend тесты: 25 passed.
- В `BACKLOG.md` задача `FE-005` помечена выполненной.
- Добавлены артефакты процесса:
  - `ADR/ADR-0025-type-details-move-form-fe-005.md`;
  - `tasks/FE-005.md`.
