...
- [x] DB-001 Реализовать схемы таблиц и ограничения
Task Context
...
- [x] DB-003 Сидирование stage_counts при создании типа
Task Context
...
- [x] API-002 `GET /api/v1/types` список типов с агрегированными counts
Task Context
...
- [x] API-003 `POST /api/v1/types` создание типа (уникальность имени)
Task Context
...
- [x] API-006 `GET /api/v1/types/{id}/history` read-time группировка
Task Context
...
- [x] API-007 `GET /api/v1/export` экспорт всего состояния
Task Context
...
- [x] FE-002 Typed API client + маппинг кодов ошибок в i18n
Task Context
...
- [x] FE-003 Main screen: список, сортировка, поиск, empty state + CTA
Task Context
...
- [x] FE-004 Создание типа (модалка/страница) + обработка дубликатов
Task Context
...
- [x] FE-005 Type details: отображение counts и форма перемещения
Task Context
...
- [x] FE-006 History UI: отображение групп
Task Context
...
- [x] CLEAN-004 Приборка этапа D + security check
Task Context
...
- [x] QA-002 Unit/Integration tests: move (конкурентность и инварианты)
Task Context
...
- [x] SEC-001 Secrets & конфигурация (безопасно по умолчанию)
Task Context
...
- [x] SEC-002 Ограничения ввода и защита от больших импортов
Task Context
...
- [x] OPS-001 Hardening контейнеров и nginx конфигурации
Task Context
...
- [x] DOCS-001 Финальная документация пользователя
Task Context
...
- [x] DOCS-002 Добавить AGENTS.md как источник инвариантов и DoD
Task Context
- В проекте декларирован приоритет `AGENTS.md` как верхнего источника правды, но файл отсутствует.
- Это повышает риск дрейфа между backlog, техспеком и фактической реализацией.
Task DOD
- Добавлен `AGENTS.md` в корень репозитория.
- Зафиксированы инварианты проекта, порядок реализации и единый DoD для задач.

- [x] CLEAN-006 Финальная приборка (dup removal, consistency, review)
Task Context
